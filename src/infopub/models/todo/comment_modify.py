...
__doc__ = '回复、评论修改的记录（被第三方修改才会有该记录） 数据定义'
...
# 数据类
class CommentModify(db.Model):
    __table_args__ = {'mysql_charset': 'utf8', 'mysql_engine': 'InnoDB'}

    id = Column(Integer, primary_key=True, doc=u'编码')

    comment_id = Column(Integer, nullable=False, index=True, doc=u'回复、评论ID')
    modifier_id = Column(Integer, nullable=False, index=True, doc=u'修改人ID')
    comment = Column(Text, nullable=False, doc=u'回复、评论')
...
# 数据管理类
class CommentModifyAdmin(PermissionView):
    # 列表视图可见列
    ...
    def __init__(self, session, name=None, category=None, endpoint=None, url=None):
        # 初始化
        super(CommentModifyAdmin, self).__init__(CommentModify, session, name, category, endpoint, url)
