...
# All rights reserved.
#
from infopub.models.material.common_file import MaterialFileQuery
...
# 数据类
class GeneralFileJsonSerializer(JsonSerializer):
    __json_hidden__ = []


# 数据类
class GeneralFile(GeneralFileJsonSerializer, db.Model, BaseMixin, TimestampMixin):
    __tablename__ = 'general_file'
    ...
    def save(self, commit=True):
        ...
        return super(GeneralFile, self).save(commit)
    ...
    def get_by_file_signature(cls, file_signature):
        return cls.query.filter(GeneralFile.file_signature == file_signature).first()


# 数据管理类
class GeneralFileAdmin(ModelAdmin):
    # 列表视图可见列
    ...
    def __init__(self, session, name=None, category=None, endpoint=None, url=None):
        # 初始化
        super(GeneralFileAdmin, self).__init__(GeneralFile, session, name, category, endpoint, url)
